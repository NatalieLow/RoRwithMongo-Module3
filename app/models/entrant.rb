class Entrant
  ...
  def update_total(_result)
      total = 0.0
      if !results.empty?
        results.each { |x| total += x[:secs] unless x[:secs].nil? }
      end
      self.secs = total
  end 
  ...
  delegate :gender, :gender=, to: :racer, prefix: true
  delegate :birth_year, :birth_year=, to: :racer
  delegate :city, :city=, to: :racer
  delegate :state, :state=, to: :racer
  delegate :name, :name=, to: :race, prefix: true
  delegate :date, :date=, to: :race, prefix: true
